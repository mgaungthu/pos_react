...
import { getPurchaseCreate, insertPurchase } from '../services';
...
export default function AddPurchasePage() {
  ...
  const { SetMessage, openModal } = useContext(ModalContext);
  ...
  const onSubmit = data => {
    ...
    insertPurchase(insertData).then(
      (response) => {
        
        setLoader(false);
        reset();
        openModal();
        SetMessage("Add purchase was successful")
                ...
                <Stack key={i} direction="row" gap={2}  >
                  <Box sx={style2}>
                    ...
                    <Stack direction="row" spacing={2}>
                      ...
                      <Box sx={{ width: "30%" }}>
                        <TextField  {...register(`order_detail.${i}.qty`, { onChange: (e) => { handleInputChange(e, i) } })}
